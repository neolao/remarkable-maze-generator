...
/**
 * The physical center of cell (x,y), in unit cell coordinates — used to place
 * the solution trace and branch-point markers for `rectangle`/
 * `rectangle-crossing` mazes (the `circle` type has its own equivalent, see
 * `circle-maze/render.ts`).
 */
export function computeCellCenter(position: { x: number; y: number }): Point {
