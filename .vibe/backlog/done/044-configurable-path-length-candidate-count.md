...
## Acceptance Criteria
- [x] User can specify the number of candidate mazes to generate and compare, as an optional parameter alongside `pathLength`, available in both the CLI (`generate`/`generate-and-send`) and the web configuration form.
- [x] When left unset, the current default behavior is preserved (10 candidates).
- [x] The option is rejected with a clear validation error when set without a `pathLength` target, or when given a non-positive/non-integer value.
- [x] The option has no effect on generation when `pathLength` is not set (no unnecessary repeated generation).
