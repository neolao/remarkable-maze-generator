...
- **Web UI**: a maze can be generated through a web API endpoint (accepting size, seed, difficulty, and solution display mode, returning the PDF directly) and through a web page form that previews the generated PDF right after submission, with invalid input caught before any request is sent. Downloading the previewed PDF and sending it to reMarkable Cloud from the web UI are not implemented yet.
