...
- A new "circle-crossing" maze type combines the circular maze layout with thick, curvy bridge crossings — two passages that tunnel through each other without connecting — available from the CLI (`--type circle-crossing`) and the web form, restricted to the growing-tree generation algorithm just like its rectangular counterpart
...
- Tunnel-crossing mazes (rectangle-crossing and circle-crossing) can now have their tube passages filled with a light gray background, making the tube shape stand out more clearly against the page — available from the CLI (`--tube-fill`) and the web form ("Fill tube background", shown only for the two crossing types), applied consistently to the PDF, the web preview, and mazes sent to reMarkable; off by default, with no effect on the classic rectangle/circle types
...
- Internal: PDF rendering, SVG preview rendering, and their shared drawing geometry now live in their own dedicated module, separate from the maze's core concepts — no user-visible change; generated mazes, PDFs, and previews are unaffected
- Internal: the reMarkable Cloud client (device pairing and PDF upload) now lives in its own dedicated module, separate from the maze's core concepts — no user-visible change; pairing and sending to reMarkable are unaffected
- Internal: the project's glossary of domain terms (maze, cell, entrance, exit, solution path, difficulty, maze type, branch point, bridge crossing, generation algorithm, path length target, ring, sector, pairing code, device token, user token) now has a real definition for every term instead of a placeholder — no user-visible or code change
...
[Unreleased]: https://github.com/neolao/remarkable-maze-generator/compare/v0.12.0...HEAD
[0.12.0]: https://github.com/neolao/remarkable-maze-generator/compare/v0.11.0...v0.12.0
[0.11.0]: https://github.com/neolao/remarkable-maze-generator/compare/v0.10.1...v0.11.0
[0.10.1]: https://github.com/neolao/remarkable-maze-generator/compare/v0.10.0...v0.10.1
